...
$(function(){
	$("#comment-submit").click(function(){
		...
		$.ajax({   
			type:"POST",
			//url:"commentArticle?articleid=" + articleid,
			url:"article/commentArticle/" + articleid+"/"+commcnt,   
			...
			success:function(data){
				if(data.code==1){
					...
					if(data.data!=$('#totalPage').text()){
						$('#totalPage').text(data.data);
					}
					$('#commcnt').text(++commcnt);
				}
